...
{
    public partial class agregarGrupoSuspendido : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //chequeo si el usuario tiene permisos para ver esta pagina
                ...
                if (Request.Cookies["IdGrupoSuspendido"] != null)
                {

                    List<AsocGrupoSolicitud> ListAsoGrupoSuspendido = new List<AsocGrupoSolicitud>();
                    ListAsoGrupoSuspendido = GrupoSuspendidoDa.ListarAsocGrupoSolicitud(Convert.ToInt32(Request.Cookies["IdGrupoSuspendido"].Value), 0, 0);
                    ViewState["AsocGrupoSolicitudList"] = ListAsoGrupoSuspendido;
                    foreach (AsocGrupoSolicitud AsocGrupoSuspendido in ListAsoGrupoSuspendido)
                    {
                        //almacenamos la data en un viewstate
                        ViewState["GrupoSuspendido"] = AsocGrupoSuspendido;

                        //Aqui se cargan todos los datos del GrupoSuspendido
                        TipoAgrupacion.SelectedIndex = AsocGrupoSuspendido.grupoSuspendido.tipoAgrupacion.id;
                        //Sacamos codigo de centro...
                        CodigoCentro.Text = AsocGrupoSuspendido.solicitudConcesion.unidadEspacial.centrosDeCultivo.codigoCentro;
                        //Obtenemos el Nombre
                        NombreGrupo.Text = AsocGrupoSuspendido.grupoSuspendido.nombreGrupoSuspend;

                        //obtenemos todos los archivos dentro del  :)
                        ViewState["ArchivoBinarioList"] = AsocGrupoSuspendido.grupoSuspendido.archivoGrupoSusp;
                        GridArchivoAdjunto.DataSource = AsocGrupoSuspendido.grupoSuspendido.archivoGrupoSusp;
                        GridArchivoAdjunto.DataBind();

                        GridViewAsociacionSolUE.DataSource = AsocGrupoSuspendido.grupoSuspendido.asocGrupoSolicitud;
                        GridViewAsociacionSolUE.DataBind();
                    }
                    //despues de cargar el Gruposuspendido matar o limpiar la cookie aunq dure 3 mins.
                    //se mata la cookie o expira dandole un tiempo menor al que tenia en un inicio, por eso se le agrega una hora menos
                    Request.Cookies["IdGrupoSuspendido"].Expires = DateTime.Now.AddHours(-1);
                }

                ViewState["paginaOrigen"] = Convert.ToString(Request.QueryString["paginaOrigen"]);
            }
        }
        ...
        protected void AgregarArchivoAdjunto_Click(object sender, ImageClickEventArgs e)
        {
            if (ArchivoAdjunto.HasFile)
            {
                ...
                else
                {
                    ...
                    {
                        Page.Validators.Add(new SubPesca.Utilidades.ValidationError("grupo2", error));
                    }
        ...
        protected void ImageButtonAsociacionSolUE_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Page.Validators.Add(new SubPesca.Utilidades.ValidationError("grupo3", error));
                }
        ...
        protected void GuardarGrupoSuspendido_Click(object sender, EventArgs e)
        {
            ...
            //CESAR 14/02/2018 se estaba cayendo si llegaba null al DA el atributo "solicitudConcesion"
            ...
            if (listaErroresGrupoSuspendido != null && listaErroresGrupoSuspendido.Count <= 0)
            {
                ...
                if (resp)
                {
                    ...
                    UpdatePanel UpdatePanelTipoPend = null;

                    try
                    {
                        string paginaOrigen = ViewState["paginaOrigen"].ToString();

                        if (paginaOrigen != null)
                        {
                            if (paginaOrigen.Equals("1")) //RECARGANDO EL CAMPO TIPO PENDIENTE DEL FORMULARIO EVALUACION ORDENAMIENTO TERRITORIAL
                            {
                                UpdatePanelTipoPend = evaluacionOrdenamientoTeritorial.Instance.UpdatePanelTipoPend;
                                evaluacionOrdenamientoTeritorial.Instance.RecargarInformacion();
                            }

                            else if (paginaOrigen.Equals("2")) //RECARGANDO EL CAMPO TIPO PENDIENTE DEL FORMULARIO DE INFORME DE CARTOGRAFÍA
                            {
                                UpdatePanelTipoPend = informeDeCartografia.Instance.UpdatePanelTipoPend;
                                informeDeCartografia.Instance.RecargarInformacion();
                              
                            }
                            else if (paginaOrigen.Equals("3")) //RECARGANDO EL CAMPO TIPO PENDIENTE DEL FORMULARIO DE INGRESAR DOCUMENTOS
                            {
                                UpdatePanelTipoPend = ingresarDocumentoComponente.Instance.UpdatePanelTipoPend;
                                ingresarDocumentoComponente.Instance.RecargarInformacion();

                            }

                            else if (paginaOrigen.Equals("4")) //RECARGANDO EL CAMPO TIPO PENDIENTE DEL FORMULARIO DE ADMINISTRACION DE DOCUMENTOS
                            {
                                UpdatePanelTipoPend = administrarDocumentoComponente.Instance.UpdatePanelTipoPend;
                                administrarDocumentoComponente.Instance.RecargarInformacion();
            ...
            else
            {
                ...
                {
                    Page.Validators.Add(new SubPesca.Utilidades.ValidationError("grupo1", error));
                }
        ...
        { 
            ...
            switch (e.CommandName)
            {
                case "Eliminar":
                    ...
                    //EliminarGrillaArchivo(idArchivo);
                    //CargaGrillaArchivo();
                    break;
                case "Descargar":
                    idArchivo = Convert.ToInt32(e.CommandArgument);
        ...
        protected void GridViewAsociacionSolUE_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (boton_eliminar != null)
                {
                    boton_eliminar.Attributes.Add("onclick", "javascript:return " + "confirm('¿Está seguro de eliminar la asociación con la unidad espacial?')");
                    boton_eliminar.Visible = true;
                };
            }
        }
        ...
        {
            int idAsociacionUE = 0;
            switch (e.CommandName)
            {
                case "Eliminar":
                    idAsociacionUE = Convert.ToInt32(e.CommandArgument);
                    GridViewAsociacionSolUE.EditIndex = -1;
                    //EliminarGrillaAsociacionSolUE(idAsociacionUE);
                    //CargaGrillaAsociacionSolUE();
                    break;
            }
        }
