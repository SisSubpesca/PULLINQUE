...
using SubPesca.Solicitudes.ECMPO;
using LogicaNegocio.cl.subpesca.rb.servicios.solicitudes;


namespace SubPesca.Unidades.ECMPO
{
    public partial class administrarCentroECMPO : System.Web.UI.Page
    {
        ...
        {
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.ADMINISTRAR_UNIDAD_ESPACIAL_ECMPO };
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (PreviousPage != null && PreviousPage is inicioSolicitudECMPO)
                {

                    if (((inicioSolicitudECMPO)PreviousPage).MensajeRegistro != null && !((inicioSolicitudECMPO)PreviousPage).MensajeRegistro.Trim().Equals(""))
                    {
                        MensajeSuperior.Text = ((inicioSolicitudECMPO)PreviousPage).MensajeRegistro;
                        PanelMensajeSuperior.Visible = true;
                        UpdatePanelMensajeSuperior.Update();
                    }
                }
                else if (PreviousPage != null && PreviousPage is unidadEspacialECMPO)
                {
                    MensajeSuperior.Text = ((unidadEspacialECMPO)PreviousPage).MensajeRegistro;
                    PanelMensajeSuperior.Visible = true;
                    UpdatePanelMensajeSuperior.Update();
                }
                ...
                // Cargamos la grilla
                Session["Filtro_Solicitud_Acuicultura_ECMPO"] = null;
                CargaGrilla();
        ...
        protected void Buscar_Click(object sender, EventArgs e)
        {
            ...
            if (!ECMPO.Text.Trim().Equals(""))
            {
                string[] split = ECMPO.Text.Trim().Split(new Char[] { ':' });
                //uso el autocompletar
                if (split != null && split.Count() > 0)
                {
                    solicitudFiltro.ecmpoFiltro = new ParametroGenerico();
                    ...
                    if (result)
                    {
                        solicitudFiltro.ecmpoFiltro.id = Convert.ToInt32(split[0]);
                    }
                    //esta buscando por un string
                    else
                    {
                        solicitudFiltro.ecmpoFiltro.id = 100000000; //no recuperara nada
                    }
                }
            }
            ...
            if (Page.IsValid)
            {
                ...
                Session["Filtro_Solicitud_Acuicultura_ECMPO"] = solicitudFiltro;
        ...
        protected void CargaGrilla()
        {
            ...
            try
            {
                solicitudFiltro = (SolicitudConcesion)Session["Filtro_Solicitud_Acuicultura_ECMPO"];
            ...
            List<SolicitudConcesion> relocalizacionTramite = solicitudDA.ListarSolicitudECMPOTraspaso(solicitudFiltro);
        ...
        protected void GridSolicitudesAdm_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "VerECMPO":
                    ...
                    Response.Redirect("~/Unidades/ECMPO/resumenECMPO.aspx");
                ...
                case "ModificarECMPO":
                    ...
                    break;
                case "Desasociar":
                    Response.Redirect("~/Unidades/ECMPO/cambiarVigenciaECMPO.aspx?idSolConcesion=" + e.CommandArgument);
                    break;
        ...
        protected void GridSolicitudesAdm_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (boton_actaEntrega != null)
                {
                    ...
                    boton_actaEntrega.Attributes.Add("onclick", "javascript:abre_dialogo('actaEntregaECMPO', '" + DataBinder.Eval(e.Row.DataItem, "idSolConcesion") + "')");
                }
                */
                ...
                if (boton_historial != null)
                {
                    if (permisosService.tieneAccesoA2((int[])ViewState["SECCION_ESPECIFICA"], (Datos.Entidades.Usuario.Serializable)Session["Usuario"], null, rbAccion.HISTORIAL))
                    {
                        ...
                        boton_historial.Attributes.Add("onclick", "javascript:abre_dialogo('historialCambioVigenciaECMPO', '" + DataBinder.Eval(e.Row.DataItem, "idSolConcesion") + "')");
                    }
                }
                ...
                if (boton_ampliarVigencia != null)
                {
                    if (permisosService.tieneAccesoA2((int[])ViewState["SECCION_ESPECIFICA"], (Datos.Entidades.Usuario.Serializable)Session["Usuario"], null, rbAccion.AMPLIAR_VIGENCIA))
                    {
                        ...
                        boton_ampliarVigencia.Attributes.Add("onclick", "javascript:abre_dialogo('extenderPlazoVigenciaECMPO', '" + DataBinder.Eval(e.Row.DataItem, "idSolConcesion") + "')");
                    }
                }
        ...
        protected void ExportarGrilla_Click(object sender, EventArgs e)
        {
            ...
            grilla.DataBind();
            SubPesca.Utilidades.GridViewExportUtil.Export("AdministrarECMPO.xls", grilla);
        }
    }
}