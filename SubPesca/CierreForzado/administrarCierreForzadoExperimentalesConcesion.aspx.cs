...
{
    public partial class administrarCierreForzadoExperimentalesConcesion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (PreviousPage != null && PreviousPage is GenerarCierreForzadoExperimentalesConcesion)
                {
                    MensajeSuperior.Text = ((GenerarCierreForzadoExperimentalesConcesion)PreviousPage).MensajeRegistro;
                ...
                if (PreviousPage != null && PreviousPage is CierreForzadoResolucionExperimentalesConcesion)
                {
                    MensajeSuperior.Text = ((CierreForzadoResolucionExperimentalesConcesion)PreviousPage).MensajeRegistro;
                    if (!MensajeSuperior.Text.Equals(""))
                    {
                        PanelMensajeSuperior.Visible = true;
                        UpdatePanelMensajeSuperior.Update();
        ...
        protected void CargaGrilla(int idInforme, string numPert)
        {
            ...
            System.Data.DataTable dataTableCierreForzado = cierreForzadoService.ListarDocGralCierreIT(idInforme, rbTipo.UNID_ESPACIAL_EXPERIMENTALES_CONCESION, rbTipo.TIPO_TRAMITE_SOLICITUD_EXPERIMENTALES_CONCESION, numPert);
        ...
        protected void GridCierreForzado_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    if (permisosService.tieneAccesoA2(new int[] { rbSeccionUnidadEspacial.ADMINISTRAR_CIERRE_FORZADO_SOLICITUD_EXPERIMENTALES_CONCESION }, (Datos.Entidades.Usuario.Serializable)Session["Usuario"], null, rbAccion.ACCESO))
                    {
        ...
        protected void GridCierreForzado_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "AsignarResolucionSSP":
                    ...
                    Response.Redirect("~/CierreForzado/CierreForzadoResolucionExperimentalesConcesion.aspx?idDocCierre=" + id);
        ...
        protected void ExportarGrilla_Click(object sender, EventArgs e)
        {
            ...
            string nom_grilla = "cierreForzadoSolicitudExperimentalesConcesion";
            string ngrilla = "";
            ...
            switch (nom_grilla)
            {
                case "cierreForzadoSolicitudExperimentalesConcesion":
                    GridCierreForzado.Columns.RemoveAt(4);
                    grilla = GridCierreForzado;
                    ngrilla = "cierreForzadoSolicitudExperimentalesConcesion.xls";
                    break;
