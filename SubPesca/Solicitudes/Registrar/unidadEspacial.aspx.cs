...
{
    public partial class unidadEspacial : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_CONCESION };
        }
        ...
        protected void GuardarUnidadEspacial_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (PlazoInicio.Text != null && !PlazoInicio.Text.Equals(""))
            {
            ...
            if (PlazoVencimiento.Text != null && !PlazoVencimiento.Text.Equals(""))
            {
       ...
       */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialConcesion(solicitudCentroDeAcopio.unidadEspacial);


            //SE VALIDA LA CREACION COMO CONCESION
            if (listaErroresUnidadEspacial == null || listaErroresUnidadEspacial.Count() == 0)
            {
                listaErroresUnidadEspacial = unidadEspacialValidacion.validaCreacionConcesionAcuicultura(solicitudCentroDeAcopio);
            }


            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.CreaUnidadEspacialConcesion(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha creado el centro acuícola exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;

                    try
                    {
                        enviarCorreo.alertaCreacionConcesion(unidadEspacial);
                    }
                    catch (Exception)
                    {
                    ...
                    this.mensaje = "Se ha creado la concesión de acuicultura " + CodigoCentro.Text + " exitosamente.";
                    Server.Transfer("~/Unidades/Concesion/administrarConcesiones.aspx");
                }
                else
                {
                    msgGrillaGral_1.Text = "No se ha creado el centro acuícola.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                }
                Ico_msgGrillaGral_1.ImageUrl = "~/App_Themes/admin_style/images/info.gif";
