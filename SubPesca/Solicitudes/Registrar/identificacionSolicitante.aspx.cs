...
using LogicaNegocio.cl.subpesca.rb.servicios.super;

namespace SubPesca.Solicitudes.Registrar
{
    public partial class identificacionSolicitante : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_CONCESION };
        ...
        */
        ...
        protected void Initialize_Form()
        {
            ...
            if (solicitudAux != null && solicitudAux.idSolConcesion > 0)
            {
                ...
                N_PERT.Value = Convert.ToString(solicitudAux.numPert);
        ...
        protected void AgregarSolicitante_Click(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                ...
                string numeroPert = Convert.ToString(N_PERT.Value);
                //string numeroPert = Request.QueryString["numeroPert"];

                // Puedes usar este valor según lo necesites en tu función
                //System.Diagnostics.Debug.WriteLine("numero pert:");
                //System.Diagnostics.Debug.WriteLine(numeroPert);

                //cambios INNCODE----------------------------------------------------------------------------cambios INNCODE
                //validacion relacion rut con CUP. en super. 
                // Asignar valor al campo CUP
                //CUP.Text = "Valor calculado o buscado";

                // Leer el valor actual del campo CUP
                string cupValue = CUP.Text;
                //solicitante.cup = cupValue;
                // Crear una instancia del servicio
                ValidacionRutCupService ValidacionCupService = new ValidacionRutCupService();

                // Parámetros para validar
                string rut = rutCompleto; // Reemplaza con el RUT
                //System.Diagnostics.Debug.WriteLine($"rut solicitane: {rut}");
                string cup = cupValue; // Reemplaza con el CUP
                //System.Diagnostics.Debug.WriteLine($"cup solicitane: {cup}");
                // Validar el CUP y el RUT
                bool CupValido = ValidacionCupService.ValidarCUP(rut, cup);

                // if (CupValido)
                //{
                //    System.Diagnostics.Debug.WriteLine("El CUP es válido y pertenece a la empresa.");
                //}
                // else
                //{
                //    System.Diagnostics.Debug.WriteLine("El CUP no es válido o no pertenece a la empresa.");
                //}
                //System.Diagnostics.Debug.WriteLine(CupValido);
                //System.Diagnostics.Debug.WriteLine(string.IsNullOrWhiteSpace(CUP.Text));

                //&& (CupValido || string.IsNullOrWhiteSpace(CUP.Text))
                //-------------------------------------------------------------------------------------cambios INNCODE
                ...
                if (solicitanteService.VerPersona(solicitante.rut,0) != null)
                {
                    ...
                    if ((solicitanteService.existeSolicitante(IdSolicitud.Value, solicitante.rut, 0) == null))
                    {
                        ...
                        //si cup es valido o si el campo CUP esta vacio se continua. 
                        if ((CupValido || string.IsNullOrWhiteSpace(CUP.Text)))
                        {
                            ...
                            // INSERTAR EN TABLA rbSupersolicitud ------ Crear una instancia del servicio
                            // Crear una instancia del servicio

                            //diferenciar si es persona Juridica, si es persona natural no se hace llamado a SUPER
                            // y ademas no se hace el insert en tabla rbSUperSolicitud
                            if (string.IsNullOrWhiteSpace(Genero.Text))
                            {
                                //System.Diagnostics.Debug.WriteLine("Campo genero esta vacio, por tanto es persona JURIDICA");
                                RbSuperSolicitudService solicitudService = new RbSuperSolicitudService();

                                // Valores simulados para probar el servicio
                                int idSuperSolicitud = 0; // 0 para una nueva inserción
                                int? idEstadoSuper = null; // Prueba con valor nulo
                                string serviceApplicationId = numeroPert; // Cambiar según los datos reales
                                int? msApplicationId = null; // Prueba con valor nulo
                                int idSolConcesion = solicitante.solicitud.idSolConcesion; // Un valor entero válido
                                string companyRut = rut; // Cambiar según los datos reales
                                string cup1 = cup; // Cambiar según los datos reales
                                DateTime? fechaModificacion = DateTime.Now; // Fecha actual
                                byte[] archivoFinal = null; // Simula un archivo en formato binario

                                try
                                {
                                    int resultado = solicitudService.InsertarOActualizarRbSuperSolicitud(
                                        idSuperSolicitud,
                                        idEstadoSuper,
                                    ...
                                    if (resultado > 0)
                                    {
                                        //System.Diagnostics.Debug.WriteLine($"Inserción/Actualización exitosa. ID generado: {resultado}");
                                        //Response.Write($"Inserción/Actualización exitosa. ID generado: {resultado}");
                                    }
                                    else
                                    {
                                        //System.Diagnostics.Debug.WriteLine("La operación no se completó.");
                                        //Response.Write("La operación no se completó.");
                                    }
                                }
                                catch (Exception ex)
                                {
                                    //System.Diagnostics.Debug.WriteLine($"Excepción capturada: {ex.Message}");
                                    //System.Diagnostics.Debug.WriteLine($"Traza de la excepción: {ex.StackTrace}");
                                    //Response.Write($"Error durante la operación: {ex.Message}");
                                }
                                // Fin insercion en tabla rbSuperSolicitud
                            }
                            ...
                            if (resp)
                            {
                                ...
                                msgGrilla.Text = "Se ha asociado el Solicitante de forma exitosa a la solicitud de concesión de acuicultura.";
                                Content_msgGrilla.Visible = true;
                            ...
                            {
                                msgGrilla.Text = "No se ha asociado el Solicitante a la solicitud de concesión de acuicultura.";
                                Content_msgGrilla.Visible = true;
                            }
                        ...
                        {
                            msgGrilla.Text = "El CUP ingresado no es válido. Si no cuenta con uno, por favor deje el campo en blanco.";
                            Content_msgGrilla.Visible = true;
                    ...
                    {
                        msgGrilla.Text = "El Solicitante ya se encuentra asociado a la solicitud de concesión de acuicultura.";
                        Content_msgGrilla.Visible = true;
                        LimpiarCamposSolicitante();
                    }
                    Ico_msgGrillaGral_1.ImageUrl = "~/App_Themes/admin_style/images/info.gif";
        ...
        protected void GridSolicitante_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "Ver":
                    Response.Redirect("~/Mantenedores/Titulares/detalleTitular.aspx?rutPersona=" + e.CommandArgument + "&bp=2");
                    break;
                case "Eliminar":
        ...
        protected void GridViewSolicitudesPendientes_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Ver":
                    ...
                    Response.Redirect("~/Solicitudes/Registrar/verDocumento.aspx?idRequerimiento=" + idRequerimiento);
