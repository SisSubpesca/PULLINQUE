...
{
    public partial class datosModificacionAmerb : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_AMERB_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_AMERB_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_AMERB_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_AMERB_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_AMERB_REGULARIZACION;
                        contador++;
                    }
        ...
        private void Initialize_Form()
        {
            ...
            List<DespliegueMenuSeccion> despliegueMenuSeccionList = despliegueMenuSeccionDA.ListarDespliegueMenuSeccion_UE(rbMenu.DATOS_MODIFICACION, 0, rbTipo.UNID_ESPACIAL_ACUICULTURA_EN_AMERB);
            ...
            if (solicitudModificacionService.despligueSeccionesMenu(solicitudModificacion, despliegueMenuSeccionList, rbSeccion.DATOS_GENERALES))
            {
                ...
                DatosSolicitudUE datosSolicitudUE = informacionAdicionalSolicitudesService.ObtieneDatosSolicitudUE(solicitudModificacion.idSolConcesion);
            ...
            if (solicitudModificacionService.despligueSeccionesMenu(solicitudModificacion, despliegueMenuSeccionList, rbSeccion.DATOS_DE_LA_CONCESION))
            {
                ...
                SolicitudConcesion solicitudConcesion = solicitudDA.ObtieneConcesionExistente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_AMERB);
        ...
        protected void GuardarSuperficieSolicitud_Click(object sender, ImageClickEventArgs e)
        {
            ...
            solicitudModificacion.tipoUnidadEspacial = new ParametroGenerico(rbTipo.UNID_ESPACIAL_ACUICULTURA_EN_AMERB);
            solicitudModificacion.tipoTramite = new ParametroGenerico(rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_AMERB);
            ...
            if (listErroresSolicitud.Count <= 0)
            {
                ...
                {
                    msgGrilla.Text = "Se han guardado las superficies a la solicitud de modificación de acuicultura en amerb.";
                    Content_msgGrilla.Visible = true;
                }
                else
                {
                    msgGrilla.Text = "No se han guardado las superficies a la solicitud de modificación de acuicultura en amerb.";
                    Content_msgGrilla.Visible = true;
