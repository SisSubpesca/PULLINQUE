...
{
    public partial class unidadesEspacialesModificacionAmerb : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["URL_ADMINISTRAR_SOLICITUD"] = paginas.URL_ADMINISTRAR_DOCUMENTO_SOLICITUD_MODIFICACION_AMERB;
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_AMERB_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_AMERB_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_AMERB_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_AMERB_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_AMERB_REGULARIZACION;
                        contador++;
                    }
                }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (permisosService.tieneAccesoA2((int[])ViewState["SECCION_ESPECIFICA"], this.usuario_logeado, solicitudConcesion, rbAccion.EDITAR))
                {
                    ...
                    PanelPlazoVencimiento.Visible = true;
                ...
                else
                {
                    ...
                    PanelPlazoVencimiento.Visible = false;
                ...
                UpdatePanelPlazoNominalVencimiento.Update();
      ...
      */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            */
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.modificaAmerbOriginal(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha modificado la Acuicultura en AMERB exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                    UpdatePanelMensajesSuperior.Update();
                ...
                {
                    msgGrillaGral_1.Text = "No se ha modificado la Acuicultura en AMERB.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                    UpdatePanelMensajesSuperior.Update();
                }
