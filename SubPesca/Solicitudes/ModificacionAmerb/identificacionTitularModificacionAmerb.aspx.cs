...
{
    public partial class identificacionTitularModificacionAmerb : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_AMERB_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_AMERB_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_AMERB_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_AMERB_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_AMERB_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_AMERB_REGULARIZACION;
                        contador++;
                    }
                }
        ...
        private void Initialize_Form()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                ...
                List<DespliegueMenuSeccion> despliegueMenuSeccionList = despliegueMenuSeccionDA.ListarDespliegueMenuSeccion_UE(rbMenu.IDENTIFICACION_TITULAR_CONCESION_MODIFICACION, 0, rbTipo.UNID_ESPACIAL_ACUICULTURA_EN_AMERB);
        ...
        private void Initialize_HT_SolicitantesAcuicultura()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                SolicitudConcesion solicitudConcesionOriginal = concesionService.ObtieneUnidadEspacialExistente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.ACUICULTURA_EN_AMERB);
        ...
        */
        ...
        protected void GridSolicitante_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "Ver":
                    Response.Redirect("~/Mantenedores/Titulares/detalleTitular.aspx?rutPersona=" + e.CommandArgument + "&bp=50");
                    break;
            };
        }
