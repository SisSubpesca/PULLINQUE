...
{
    public partial class inicioSolicitudAmerb : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (permisosService.tieneAccesoA2(new int[] { rbSeccionUnidadEspacial.INGRESAR_SOLICITUD_AMERB }, (Datos.Entidades.Usuario.Serializable)Session["Usuario"], null, rbAccion.EDITAR))
                {
        ...
        {
            SolicitudCentroAmerbService centroAmerbService = new SolicitudCentroAmerbService();
            ...
            List<String> listErroresSolicitudInicial = inicioSolicitudConcesionValidacion.validaInicioSolicitudAcuiculturaAmerb(solicitudInicial);

            if (listErroresSolicitudInicial.Count <= 0)
            {
                if (centroAmerbService.guardarSolicitudAcuiculturaAmerb(solicitudInicial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario))
                {
                    ...
                    Session["SolicitudCentroAmerb"] = (SolicitudConcesion)solicitudInicial;
