...
{
    public partial class unidadEspacialAmerb : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["URL_ADMINISTRAR_SOLICITUD"] = paginas.URL_ADMINISTRAR_DOCUMENTO_SOLICITUD_AMERB;
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_AMERB };
        }
        ...
        protected void GuardarUnidadEspacial_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (NumeroDiarioOficial != null && !NumeroDiarioOficial.Text.Trim().Equals(""))
            {
                unidadEspacial.numeroDiarioOficial = Convert.ToInt32(NumeroDiarioOficial.Text);
            }


            if (FechaDiarioOficial != null && !FechaDiarioOficial.Text.Trim().Equals(""))
            {
                unidadEspacial.fechaDiarioOficial = Convert.ToDateTime(FechaDiarioOficial.Text);
            }

            if (NumeroActaEntrega != null && !NumeroActaEntrega.Text.Trim().Equals(""))
            {
                unidadEspacial.numeroActaEntrega = Convert.ToInt32(NumeroActaEntrega.Text);
            }
            if (FechaActaEntrega != null && !FechaActaEntrega.Text.Trim().Equals(""))
            {
            ...
            //unidadEspacial.capitaniaDePuerto.idCapitaDePuerto = Convert.ToInt32(CapitaniaPuerto.SelectedItem.Value);


            unidadEspacial.tipoPlazoNominal = new ParametroGenerico(Convert.ToInt32(PlazoNominal.SelectedItem.Value));

            if (PlazoInicio != null && !PlazoInicio.Text.Trim().Equals(""))
            {
                unidadEspacial.plazoInicio = Convert.ToDateTime(PlazoInicio.Text);
            }

            if (NumeroPlazo != null && !NumeroPlazo.Text.Trim().Equals("")) 
            {
                unidadEspacial.numPlazo = Convert.ToInt32(NumeroPlazo.Text);
            }

            if (PlazoVencimiento != null && !PlazoVencimiento.Text.Trim().Equals(""))
            {
                unidadEspacial.plazoVencimiento = Convert.ToDateTime(PlazoVencimiento.Text);
            }

            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialAmerb(unidadEspacial);
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                ...
                bool resp = unidadEspacialService.guardaUnidadEspacialAmerb(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {
      ...
      */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            SolicitudConcesion solicitudCentroDeAmerb = (SolicitudConcesion)Session[paginas.solicitudAmerbSession];
            solicitudCentroDeAmerb.unidadEspacial = unidadEspacial;


            UnidadEspacial unidadEspacialAux = (UnidadEspacial)ViewState["unidadEspacial"];
            solicitudCentroDeAmerb.unidadEspacial.idUnidadEspacial = unidadEspacialAux.idUnidadEspacial;


            //SE VALIDA LA UNIDAD ESPACIAL
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialAmerb(solicitudCentroDeAmerb.unidadEspacial);
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.CreaUnidadEspacialAmerb(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {
                    ...
                    /*
                    msgGrillaGral_1.Text = "Se ha creado la concesión de acuicultura exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                    */
                    ...
                    try
                    {
                        enviarCorreo.alertaCreacionAmerb(unidadEspacial);
                    }catch(Exception ex){
                    ...
                    this.mensaje = "Se ha creado la acuicultura en amerb " + CodigoCentro.Text + " exitosamente.";
                    Server.Transfer("~/Unidades/Amerb/administrarCentroAmerb.aspx");
