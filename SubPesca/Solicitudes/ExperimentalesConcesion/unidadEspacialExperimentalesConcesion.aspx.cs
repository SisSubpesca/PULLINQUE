...
{
    public partial class unidadEspacialExperimentalesConcesion : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_EXPERIMENTALES_CONCESION };
        }
        ...
        protected void GuardarUnidadEspacial_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (NumeroActaEntrega != null && !NumeroActaEntrega.Text.Trim().Trim().Equals(""))
            {
            ...
            if (NumeroPlazo.Text != null && !NumeroPlazo.Text.Trim().Equals(""))
            {
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialExperimentalesConcesion(unidadEspacial);
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                ...
                bool resp = unidadEspacialService.guardaUnidadEspacialExperimentalesConcesion(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {
                    ViewState["unidadEspacial"] = unidadEspacial;
       ...
       */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            SolicitudConcesion solicitudExperimentalConcesion = (SolicitudConcesion)Session[ViewState["solicitudSession"].ToString()];
            solicitudExperimentalConcesion.unidadEspacial = unidadEspacial;


            //SE VALIDA LA UNIDAD ESPACIAL
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialExperimentalesConcesion(solicitudExperimentalConcesion.unidadEspacial);


            //SE VALIDA LA CREACION COMO CONCESION
            if (listaErroresUnidadEspacial == null || listaErroresUnidadEspacial.Count() == 0)
            {
                listaErroresUnidadEspacial = unidadEspacialValidacion.validaCreacionExperimentalesConcesion(solicitudExperimentalConcesion);
            }


            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.CreaUnidadEspacialExperimentalesConcesion(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha creado la Experimental Concesión exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;

                    try
                    {
                        enviarCorreo.alertaCreacionExperimentalesConcesion(unidadEspacial);
                    }
                    catch (Exception)
                    {
                ...
                {
                    msgGrillaGral_1.Text = "No se ha creado la Experimental Concesión.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                }
                Ico_msgGrillaGral_1.ImageUrl = "~/App_Themes/admin_style/images/info.gif";
