...
{
    public partial class identificacionTitularConcesion : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CONCESION_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CONCESION_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CONCESION_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CONCESION_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CONCESION_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CONCESION_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CONCESION_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CONCESION_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CONCESION_REGULARIZACION;
                        contador++;
                    }
                }
        ...
        private void Initialize_HT_SolicitantesAcuicultura()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                SolicitudConcesion solicitudConcesionOriginal = concesionService.ObtieneConcesionExistente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.TIPO_TRAMITE_SOLICITUD_CONCESION_ACUICULTURA);
        ...
        */
        ...
        protected void GridSolicitante_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "Ver":
                    Response.Redirect("~/Mantenedores/Titulares/detalleTitular.aspx?rutPersona=" + e.CommandArgument + "&bp=1");
                    break;
            };
        }
