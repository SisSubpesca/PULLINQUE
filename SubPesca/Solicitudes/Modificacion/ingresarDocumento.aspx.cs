...
namespace SubPesca.Solicitudes.Modificacion
{
    public partial class ingresarDocumento : System.Web.UI.Page
    {
        ...
        protected void NumeroCI_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                List<SolicitudConcesion> solicitudes = requerimientoService.ObtenerSolicitudPorCIusado(Convert.ToInt32(IdSolicitud.Text), Convert.ToInt32(NumeroCI.Text.Trim()));
        ...
        private void controlarCamposLogicos(int idTipoIO)
        {
            ...
            if (idTipoIO == rbTipo.RESPUESTA_A_UN_REQUERIMIENTO)
            {
                ...
                DataTable data = requerimientoService.ListarRequerimientosIdSolicitud(Convert.ToInt32(IdSolicitud.Text), dbSeccion, Convert.ToInt32(Origen.SelectedValue));
                if (data != null)
                {
        ...
        protected void ListViewEntradaRespuestaRequerimiento_Carga()
        {
            ...
            if (Convert.ToInt32(NRequerimiento.SelectedValue) > 0)
            {
                respu = requerimientoService.ListarDocumentacionPestanaReqMod(Convert.ToInt32(NRequerimiento.SelectedValue));
            ...
            switch (e.CommandName)
            {
                ...
                case "Eliminar":
                    ...
                    if (idTipoFlujoDocumental == rbTipo.ENTRADA)
                    {
                        ...
                        if (errores.Count == 0)
                        {
                            requerimientoService.EliminarRespuesta(idRequerimiento, idPestana, requerimiento.solicitud.idSolConcesion);
                        }
                        else
                        {
                    ...
                    {
                        Requerimiento requerimiento = requerimientoService.ObtenerRequerimiento(idRequerimiento);
                        List<String> errores = ingresarDocumentoValidacion.validarEliminacionDeRequerimiento(requerimiento);
                        if (errores.Count == 0)
                        {
                            requerimientoService.EliminarRequerimiento(idRequerimiento, idPestana, requerimiento.solicitud.idSolConcesion);
                        }
                        else
                        {
                ...
                case "NoVigente":
                    ...
                    if (idTipoFlujoDocumental == rbTipo.ENTRADA)
                    {
                        ...
                        if (errores.Count == 0)
                        {
                            requerimientoService.ActualizarRespuestaEstado(idRequerimiento, idPestana, rbEstadosGenerales.NO_VIGENTE);
                        }
                        else
                        {
                    ...
                    if (idTipoFlujoDocumental == rbTipo.SALIDA)
                    {
                        ...
                        if (errores.Count == 0)
                        {
                            requerimientoService.ActualizarRequerimientoEstado(idRequerimiento, idPestana, rbEstadosGenerales.NO_VIGENTE);
                        }
                        else
                        {
                ...
                case "Vigente":
                    ...
                    if (idTipoFlujoDocumental == rbTipo.ENTRADA)
                    {
                        ...
                        if (errores.Count == 0)
                        {
                            requerimientoService.ActualizarRespuestaEstado(idRequerimiento, idPestana, rbEstadosGenerales.VIGENTE);
                        }
                        else
                        {
                    ...
                    if (idTipoFlujoDocumental == rbTipo.SALIDA)
                    {
                        ...
                        if (errores.Count == 0)
                        {
                            requerimientoService.ActualizarRequerimientoEstado(idRequerimiento, idPestana, rbEstadosGenerales.VIGENTE);
                        }
                        else
                        {
        ...
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                ...
                if (Page.IsValid)
                {
                    ...
                    if (!Numero.Text.Trim().Equals(""))
                    {
                        requerimiento.numero = Convert.ToInt32(Numero.Text);
                    }
                    ...
                    if (errores.Count == 0)
                    {
                        ...
                        bool resp = requerimientoService.guardarRequerimiento(requerimiento);
