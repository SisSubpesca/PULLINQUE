...
{
    public partial class redefinirSolicitudRelocalizacionRESA : System.Web.UI.Page
    {
        ConcesionService concesionService = new ConcesionService();
        PermisosService permisosService = new PermisosService();
        TramiteRelocalizacionRESAValidacion tramiteRelocalizacionValidacionRESA = new TramiteRelocalizacionRESAValidacion();
        ...
        protected void GuardarOrigen_Click(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                ...
                else
                {
                    ...
                    List<String> errores = tramiteRelocalizacionValidacionRESA.validaIngresoOrigenSectorRESA(nuevoOrigen, origenes, false);
        ...
        protected void ModificarOrigen(int indexModificar)
        {
            ...
            if (Page.IsValid)
            {
                ...
                List<String> errores = tramiteRelocalizacionValidacionRESA.validaModificarOrigenSectorRESA(origenModificado, origenes, indexModificar);
        ...
        protected void GridOrigen_Eliminar(int index)
        {
            ...
            foreach (OrigenSector aOrigenSector in origenes)
            {
                if (aOrigenSector.index == index)
                {
                    List<String> errores = tramiteRelocalizacionValidacionRESA.validaEliminacionOrigenSectorRESA(aOrigenSector);
                    if (errores.Count > 0)
                    {
                        foreach (String error in errores)
                        {
            ...
            else
            {
                ...
                if (Page.IsValid)
                {
                    ...
                    if (nuevoSector.esSectorCero)
                    {
                        ...
                        errores = tramiteRelocalizacionValidacionRESA.validaIngresoOrigenSectorRESA(origenSector, null, true);
                    ...
                    {
                        errores = tramiteRelocalizacionValidacionRESA.validaIngresoSectorRelocalizacionRESA(nuevoSector, sectores);
                    }

                    if (errores.Count == 0)
                    {

                        nuevoSector.numSector = this.obtenerNumeroSector();
        ...
        private int obtenerNumeroSector()
        {
        ...
        protected void ModificarSector(int indexModificar)
        {
            ...
            if (Page.IsValid)
            {
                ...
                if (nuevoSector.esSectorCero)
                {
                    ...
                    nuevoSector.tipoRelocalizacion = new ParametroGenerico(rbTipo.RELOCALIZACION_SECTOR_CERO_RESA);
                ...
                if (errores == null || errores.Count == 0)
                {
                    errores = tramiteRelocalizacionValidacionRESA.validaModificarSectorRelocalizacionRESA(nuevoSector, sectores, indexModificar);
                }
        ...
        protected void GuardarTramite_Click(object sender, EventArgs e)
        {
            ...
            List<String> errores = tramiteRelocalizacionValidacionRESA.validaTramiteRelocalizacionRESA(tramiteRelocalizacion);


            if (errores.Count == 0)
            {

                if (relocalizacionRESAService.RedefinirTramiteRelocalizacionRESA(tramiteRelocalizacion, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario))
                {
