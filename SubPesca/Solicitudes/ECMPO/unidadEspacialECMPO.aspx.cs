...
{
    public partial class unidadEspacialECMPO : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_ECMPO };
        }
        ...
        protected void GuardarUnidadEspacial_Click(object sender, ImageClickEventArgs e)
        {
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialECMPO(unidadEspacial);
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                ...
                bool resp = unidadEspacialService.guardaUnidadEspacialECMPO(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {
                    ViewState["unidadEspacial"] = unidadEspacial;
       ...
       */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialECMPO(solicitudCentroDeAcopio.unidadEspacial);


            //SE VALIDA LA CREACION COMO CONCESION
            if (listaErroresUnidadEspacial == null || listaErroresUnidadEspacial.Count() == 0)
            {
                listaErroresUnidadEspacial = unidadEspacialValidacion.validaCreacionECMPO(solicitudCentroDeAcopio);
            }


            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.CreaUnidadEspacialECMPO(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha creado la acuicultura en ECMPO exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;

                    try
                    {
                        enviarCorreo.alertaCreacionCentroECMPO(solicitudCentroDeAcopio);
                    }
                    catch (Exception)
                    {
                    ...
                    this.mensaje = "Se ha creado la acuicultura en ECMPO " + CodigoCentro.Text + " exitosamente.";
                    Server.Transfer("~/Unidades/ECMPO/administrarCentroECMPO.aspx");
                }
                else
                {
                    msgGrillaGral_1.Text = "No se ha creado la acuicultura en ECMPO.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                }
                Ico_msgGrillaGral_1.ImageUrl = "~/App_Themes/admin_style/images/info.gif";
