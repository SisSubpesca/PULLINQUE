...
{
    public partial class verSolicitudRelocalizacion : System.Web.UI.Page
    {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (Request.QueryString["idTramiteRel"] != null)
                {
                    ...
                    if (tramiteRelocalizacion != null && tramiteRelocalizacion.idTramiteRel > 0)
                    {
                        ...
                        ViewState["Sectores_Tramite"] = (List<DetalleSector>)tramiteRelocalizacion.sectores;
                ...
                if (Page.IsValid)
                {
                    ...
                    if (nuevoSector.esSectorCero)
                    {
                        ...
                        if (!HectareasSectorCero.Text.Trim().Equals(""))
                        {
                            try
                            {
                                origenSector.superficieRelocalizada = Convert.ToSingle(HectareasSectorCero.Text);
                            }
                            catch (Exception) { }
        ...
        protected void ModificarSector(int indexModificar)
        {
            ...
            if (Page.IsValid)
            {
                ...
                if (nuevoSector.esSectorCero)
                {
                    ...
                    nuevoSector.tipoRelocalizacion = new ParametroGenerico(rbTipo.RELOCALIZACION_SECTOR_CERO);
        ...
        protected void GuardarTramite_Click(object sender, EventArgs e)
        {
            ...
            if (errores.Count == 0)
            {
                ...
                if (relocalizacionService.GuardarTramiteRelocalizacion(tramiteRelocalizacion, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario))
                {
                    Response.Redirect("~/Solicitudes/Relocalizacion/administrarSolicitudRelocalizacion.aspx");
                }
                else
                {
                    Page.Validators.Add(new ValidationError("GrupoSector", "Ha ocurrido un error al realizacion la acción solicitada"));
                }
            }
            else
            {
