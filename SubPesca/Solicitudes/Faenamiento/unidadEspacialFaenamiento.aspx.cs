...
{
    public partial class unidadEspacialFaenamiento : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_FAENAMIENTO };
        }
        ...
        protected void GuardarUnidadEspacial_Click(object sender, ImageClickEventArgs e)
        {
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialFaenamiento(unidadEspacial);
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                ...
                bool resp = unidadEspacialService.guardaUnidadEspacialFaenamiento(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {
                    ViewState["unidadEspacial"] = unidadEspacial;
       ...
       */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            SolicitudConcesion solicitudCentroDeAcopio = (SolicitudConcesion)Session[ViewState["solicitudSession"].ToString()];
            solicitudCentroDeAcopio.unidadEspacial = unidadEspacial;


            //SE VALIDA LA UNIDAD ESPACIAL
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialFaenamiento(solicitudCentroDeAcopio.unidadEspacial);


            //SE VALIDA LA CREACION COMO CONCESION
            if (listaErroresUnidadEspacial == null || listaErroresUnidadEspacial.Count() == 0)
            {
                listaErroresUnidadEspacial = unidadEspacialValidacion.validaCreacionConcesionFaenamiento(solicitudCentroDeAcopio);
            }


            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.CreaUnidadEspacialFaenamiento(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha creado el centro de faenamiento exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;

                    try
                    {
                        enviarCorreo.alertaCreacionCentroFaenamiento(solicitudCentroDeAcopio);
                    }
                    catch (Exception)
                    {
                    ...
                    this.mensaje = "Se ha creado el centro de faenamiento " + CodigoCentro.Text + " exitosamente.";
                    Server.Transfer("~/Unidades/Faenamiento/administrarCentroFaenamiento.aspx");
                }
                else
                {
                    msgGrillaGral_1.Text = "No se ha creado el centro de faenamiento.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                }
                Ico_msgGrillaGral_1.ImageUrl = "~/App_Themes/admin_style/images/info.gif";
