...
{
    public partial class ingresarSolicitudModificacionCentroAcopio : System.Web.UI.Page
    {
        ...
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ...
            solicitudInicial.tipoUnidadEspacial = new ParametroGenerico(rbTipo.UNID_ESPACIAL_MOD_CENTRO_ACOPIO);
            solicitudInicial.tipoTramite = new ParametroGenerico(rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_CENTRO_ACOPIO);
            ...
            if (Especie.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_ACOPIO_RENOVACION));
            }

            if (ProyectoTecnico.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_ACOPIO_PT_ESPECIE));
            }

            if (ampliacionSuperficie.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_ACOPIO_AMPLIA_SUPERFICIE));
            }

            if (reduccionSuperficie.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_ACOPIO_REDUCE_SUPERFICIE));
            }

            if (regularizacion.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_ACOPIO_REGULARIZACION));
            }

            List<String> listErroresSolicitudInicial = ingresarSolicitudModificacionValidacion.validaIngresarSolicitudModificacionAcopio(solicitudInicial);

            if (Page.IsValid && listErroresSolicitudInicial.Count <= 0)
            {
                if (solicitudModificacionService.guardarSolicitudModificacionAcopioInicial(solicitudInicial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario))
                {
                    ...
                    solicitudDB.tipoTramite = new ParametroGenerico(rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_CENTRO_ACOPIO);


                    Session["SolicitudModificacionCentroAcopio"] = (SolicitudConcesion)solicitudDB;
        ...
        protected void CodigoCentro_OnTextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Page.IsValid)
                {
                    List<String> listErroresSolicitudInicial = ingresarSolicitudModificacionValidacion.validaCodigoCentroCentroAcopio(CodigoCentro.Text);
        ...
        protected void TitularCentro_OnTextChanged(object sender, EventArgs e)
        {
              ...
              try
            {
                ...
                if (Page.IsValid)
                {
                    List<String> listErroresSolicitudInicial = ingresarSolicitudModificacionValidacion.validaTitularCentroAcopio(CodigoCentro.Text, TitularCentro.Text);
