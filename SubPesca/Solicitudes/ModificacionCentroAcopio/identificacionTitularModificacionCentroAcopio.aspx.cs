...
{
    public partial class identificacionTitularModificacionCentroAcopio : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CENTRO_ACOPIO_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_ACOPIO_RENOVACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CENTRO_ACOPIO_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_ACOPIO_PT_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CENTRO_ACOPIO_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_ACOPIO_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CENTRO_ACOPIO_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_ACOPIO_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_CENTRO_ACOPIO_REGULARIZACION;
                        contador++;
                    }
                }
        ...
        private void Initialize_Form()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                ...
                List<DespliegueMenuSeccion> despliegueMenuSeccionList = despliegueMenuSeccionDA.ListarDespliegueMenuSeccion_UE(rbMenu.IDENTIFICACION_TITULAR_CONCESION_MODIFICACION, 0, rbTipo.UNID_ESPACIAL_MOD_CENTRO_ACOPIO);
        ...
        private void Initialize_HT_SolicitantesAcuicultura()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                SolicitudConcesion solicitudConcesionOriginal = concesionService.ObtieneUnidadEspacialExistente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.UNID_ESPACIAL_CENTRO_DE_ACOPIO);
        ...
        */
        ...
        protected void GridSolicitante_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "Ver":
                    Response.Redirect("~/Mantenedores/Titulares/detalleTitular.aspx?rutPersona=" + e.CommandArgument + "&bp=51");
                    break;
            };
        }
