...
{
    public partial class unidadEspacialExperimentalesAmerb : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["SECCION_ESPECIFICA"] = new int[] { rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_EXPERIMENTALES_AMERB };
        }
        ...
        protected void GuardarUnidadEspacial_Click(object sender, ImageClickEventArgs e)
        {
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialExperimentalesAmerb(unidadEspacial);
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                ...
                bool resp = unidadEspacialService.guardaUnidadEspacialExperimentalesAmerb(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {
                    ViewState["unidadEspacial"] = unidadEspacial;
       ...
       */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            SolicitudConcesion solicitudExperimentalAmerb = (SolicitudConcesion)Session[ViewState["solicitudSession"].ToString()];
            solicitudExperimentalAmerb.unidadEspacial = unidadEspacial;


            //SE VALIDA LA UNIDAD ESPACIAL
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialExperimentalesAmerb(solicitudExperimentalAmerb.unidadEspacial);


            //SE VALIDA LA CREACION COMO CONCESION
            if (listaErroresUnidadEspacial == null || listaErroresUnidadEspacial.Count() == 0)
            {
                listaErroresUnidadEspacial = unidadEspacialValidacion.validaCreacionExperimentalesAmerb(solicitudExperimentalAmerb);
            }


            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.CreaUnidadEspacialExperimentalesAmerb(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha creado la Experimental Amerb exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;

                    try
                    {
                        enviarCorreo.alertaCreacionExperimentalesAmerb(unidadEspacial);
                    }
                    catch (Exception)
                    {
                ...
                {
                    msgGrillaGral_1.Text = "No se ha creado la Experimental Amerb.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                }
                Ico_msgGrillaGral_1.ImageUrl = "~/App_Themes/admin_style/images/info.gif";
