...
{
    public partial class identificacionTitularModificacionECMPO : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_ECMPO_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_ECMPO_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_ECMPO_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_ECMPO_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.IDENTIFICACION_SOLICITANTE_MOD_ECMPO_REGULARIZACION;
                        contador++;
                    }
                }
        ...
        private void Initialize_Form()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                ...
                List<DespliegueMenuSeccion> despliegueMenuSeccionList = despliegueMenuSeccionDA.ListarDespliegueMenuSeccion(rbMenu.IDENTIFICACION_TITULAR_CONCESION_MODIFICACION, 0);
        ...
        private void Initialize_HT_SolicitantesAcuicultura()
        {
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                //SolicitudConcesion solicitudConcesionOriginal = concesionService.ObtieneConcesionExistente(solicitudModificacion.tramiteModConcesion.centro.id, rbTipo.UNID_ESPACIAL_ECMPO);

                SolicitudConcesion solicitudConcesionOriginal = solicitudDA.Obtiene_UE_Existente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.UNID_ESPACIAL_ECMPO);
        ...
        */
        ...
        protected void GridSolicitante_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                case "Ver":
                    Response.Redirect("~/Mantenedores/Titulares/detalleTitular.aspx?rutPersona=" + e.CommandArgument + "&bp=53");
                    break;
            };
        }
