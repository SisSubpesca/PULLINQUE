...
{
    public partial class unidadesEspacialesModificacionECMPO : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_ECMPO_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_ECMPO_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_ECMPO_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_ECMPO_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_ECMPO_REGULARIZACION;
                        contador++;
                    }
                }
            ...
            if (solicitudModificacion != null && solicitudModificacion.idSolConcesion > 0)
            {
                ...
                if (unidadespacial != null)
                {
                    ...
                    if (!solicitudModificacionService.tieneSoloTipoModificacionRegularizacion(solicitudModificacion))
                    {
                        ...
                        PanelGuardarUnidadEspacial.Visible = true;
                        UpdatePanelGuardarUnidadEspacial.Update();
                        PanelBotonLimpiar.Visible = true;

                        PanelModConcesion.Visible = true;
                        UpdatePanelModConcesion.Update();
                    }
                    else if (solicitudModificacionService.tieneSoloTipoModificacionRegularizacion(solicitudModificacion)) {
                        NumeroDiarioOficial.Text = Convert.ToString(unidadespacial.numeroDiarioOficial);
                        NumeroDiarioOficial.ReadOnly = false;

                        FechaDiarioOficial.Text = FechaUtils.formatearFecha(unidadespacial.fechaDiarioOficial);
                        FechaDiarioOficial.ReadOnly = false;
                        PanelCalendario.Visible = false;
                        ...
                        PanelModConcesion.Visible = false;
                        UpdatePanelModConcesion.Update();
                    }
       ...
       */
        ...
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {
                bool resp = unidadEspacialService.modificaECMPOOriginal(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    IdUnidadEspacial.Value = Convert.ToString(unidadEspacial.idUnidadEspacial);

                    msgGrillaGral_1.Text = "Se ha modificado el ECMPO exitosamente.";
                ...
                {
                    msgGrillaGral_1.Text = "No se ha modificado el ECMPO.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                }
