...
{
    public partial class datosModificacionECMPO : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_ECMPO_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_ECMPO_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_PT)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_ECMPO_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_ECMPO_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_ECMPO_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_ECMPO_REGULARIZACION;
                        contador++;
                    }
        ...
        private void Initialize_Form()
        {
            ...
            List<DespliegueMenuSeccion> despliegueMenuSeccionList = despliegueMenuSeccionDA.ListarDespliegueMenuSeccion(rbMenu.DATOS_MODIFICACION, 0);
            ...
            if (solicitudModificacionService.despligueSeccionesMenu(solicitudModificacion, despliegueMenuSeccionList, rbSeccion.DATOS_DE_LA_CONCESION))
            {
                ...
                SolicitudConcesion solicitudConcesion = solicitudDA.ObtieneConcesionExistente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.TIPO_TRAMITE_SOLICITUD_CONCESION_ACUICULTURA);
            ...
            {
                List<SolicitudConcesion> solicitudConcesionList = solicitudModificacionService.ListarPequerimientosPendCodCentro(solicitudModificacion.tramiteModConcesion.centro.id, solicitudModificacion.idSolConcesion);
        ...
        protected void GuardarSuperficieSolicitud_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (listErroresSolicitud.Count <= 0)
            {
                ...
                {
                    msgGrilla.Text = "Se han guardado las superficies a la solicitud de modificación de ECMPO.";
                    Content_msgGrilla.Visible = true;
                }
                else
                {
                    msgGrilla.Text = "No se han guardado las superficies a la solicitud de modificación de ECMPO.";
                    Content_msgGrilla.Visible = true;
