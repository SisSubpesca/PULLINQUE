...
{
    public partial class unidadesEspacialesModificacionCentroFaenamiento : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            ViewState["URL_ADMINISTRAR_SOLICITUD"] = paginas.URL_ADMINISTRAR_DOCUMENTO_SOLICITUD_MODIFICACION_CENTRO_FAENAMIENTO;
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_CENTRO_FAENAMIENTO_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_RENOVACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_CENTRO_FAENAMIENTO_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_PT_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_CENTRO_FAENAMIENTO_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_CENTRO_FAENAMIENTO_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.UNIDAD_ESPACIAL_MOD_CENTRO_FAENAMIENTO_REGULARIZACION;
                        contador++;
                    }
                }
      ...
      */
        protected void CrearConcesion_Click(object sender, ImageClickEventArgs e)
        {
            ...
            */
            ...
            List<String> listaErroresUnidadEspacial = unidadEspacialValidacion.validaUnidadEspacialFaenamiento(solicitudCentroDeAmerb.unidadEspacial);
            ...
            */
            ...
            if (listaErroresUnidadEspacial != null && listaErroresUnidadEspacial.Count <= 0)
            {
                bool resp = unidadEspacialService.modificaCentroFaenamientoOriginal(unidadEspacial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario);
                if (resp)
                {

                    ViewState["unidadEspacial"] = unidadEspacial;

                    msgGrillaGral_1.Text = "Se ha modificado el centro de faenamiento exitosamente.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                    UpdatePanelMensajesSuperior.Update();
                    ...
                    try
                    {
                        enviarCorreo.alertaCreacionCentroFaenamiento(solicitudCentroDeAmerb);
                    }
                    catch (Exception)
                    {
                ...
                {
                    msgGrillaGral_1.Text = "No se ha modificado el centro de faenamiento.";
                    msgGrillaGral_1.Focus();
                    Content_msgGrillaGral_1.Visible = true;
                    UpdatePanelMensajesSuperior.Update();
                }
