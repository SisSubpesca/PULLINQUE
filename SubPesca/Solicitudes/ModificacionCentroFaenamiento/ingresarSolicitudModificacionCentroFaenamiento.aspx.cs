...
namespace SubPesca.Solicitudes.ModificacionCentroFaenamiento
{
    ...
    public partial class ingresarSolicitudModificacionCentroFaenamiento : System.Web.UI.Page
    {
        ...
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ...
            solicitudInicial.tipoUnidadEspacial = new ParametroGenerico(rbTipo.UNID_ESPACIAL_MOD_CENTRO_FAENAMIENTO);
            solicitudInicial.tipoTramite = new ParametroGenerico(rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_CENTRO_FAENAMIENTO);
            ...
            if (Especie.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_FAENAMIENTO_RENOVACION));
            }

            if (ProyectoTecnico.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_FAENAMIENTO_PT_ESPECIE));
            }

            if (ampliacionSuperficie.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_FAENAMIENTO_AMPLIA_SUPERFICIE));
            }

            if (reduccionSuperficie.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_FAENAMIENTO_REDUCE_SUPERFICIE));
            }

            if (regularizacion.Checked)
            {
                solicitudInicial.tipoModificacionesTram.Add(new ParametroGenerico(rbTipo.MOD_CENTRO_FAENAMIENTO_REGULARIZACION));
            }

            List<String> listErroresSolicitudInicial = ingresarSolicitudModificacionValidacion.validaIngresarSolicitudModificacionFaenamiento(solicitudInicial);

            if (Page.IsValid && listErroresSolicitudInicial.Count <= 0)
            {
                if (solicitudModificacionService.guardarSolicitudModificacionFaenamientoInicial(solicitudInicial, Session["Usuario"] == null ? 0 : ((Datos.Entidades.Usuario.Serializable)Session["Usuario"]).id_usuario))
                {
                    ...
                    solicitudDB.tipoTramite = new ParametroGenerico(rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_CENTRO_FAENAMIENTO);


                    Session["SolicitudModificacionCentroFaenamiento"] = (SolicitudConcesion)solicitudDB;
        ...
        protected void CodigoCentro_OnTextChanged(object sender, EventArgs e)
        {
             ...
             try
            {
                ...
                if (Page.IsValid)
                {
                    List<String> listErroresSolicitudInicial = ingresarSolicitudModificacionValidacion.validaCodigoCentroCentroFaenamiento(CodigoCentro.Text);
        ...
        protected void TitularCentro_OnTextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Page.IsValid)
                {
                    List<String> listErroresSolicitudInicial = ingresarSolicitudModificacionValidacion.validaTitularCentroFaenamiento(CodigoCentro.Text, TitularCentro.Text);
