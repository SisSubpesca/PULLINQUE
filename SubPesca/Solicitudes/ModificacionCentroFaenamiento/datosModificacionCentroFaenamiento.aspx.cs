...
{
    public partial class datosModificacionCentroFaenamiento : System.Web.UI.Page
    {
        ...
        protected void setearModulo()
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                foreach (ParametroGenerico tipoModificacion in solicitudConcesion.tipoModificacionesTram)
                {
                    ...
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_CENTRO_FAENAMIENTO_AMPLIA_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_RENOVACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_CENTRO_FAENAMIENTO_ESPECIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_PT_ESPECIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_CENTRO_FAENAMIENTO_PT;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_REDUCE_SUPERFICIE)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_CENTRO_FAENAMIENTO_REDUCE_SUPERFICIE;
                        contador++;
                    }
                    if (tipoModificacion.id == rbTipo.MOD_CENTRO_FAENAMIENTO_REGULARIZACION)
                    {
                        tiposModificacion[contador] = rbSeccionUnidadEspacial.FICHA_MODIFICACION_CENTRO_FAENAMIENTO_REGULARIZACION;
                        contador++;
                    }
        ...
        private void Initialize_Form()
        {
            ...
            List<DespliegueMenuSeccion> despliegueMenuSeccionList = despliegueMenuSeccionDA.ListarDespliegueMenuSeccion_UE(rbMenu.DATOS_MODIFICACION, 0, rbTipo.UNID_ESPACIAL_MOD_CENTRO_FAENAMIENTO);
            ...
            if (solicitudModificacionService.despligueSeccionesMenu(solicitudModificacion, despliegueMenuSeccionList, rbSeccion.DATOS_DE_LA_CONCESION))
            {
                ...
                SolicitudConcesion solicitudConcesion = solicitudDA.Obtiene_UE_Existente(Convert.ToString(solicitudModificacion.tramiteModConcesion.centro.id), rbTipo.UNID_ESPACIAL_CENTRO_DE_FAENAMIENTO);
            ...
            {
                List<SolicitudConcesion> solicitudConcesionList = solicitudModificacionService.ListarRequerimientosPendModificacionCodCentro(solicitudModificacion.tramiteModConcesion.centro.id, solicitudModificacion.idSolConcesion, rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_CENTRO_FAENAMIENTO, rbTipo.UNID_ESPACIAL_MOD_CENTRO_FAENAMIENTO);
        ...
        protected void GuardarSuperficieSolicitud_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (listErroresSolicitud.Count <= 0)
            {
                ...
                {
                    msgGrilla.Text = "Se han guardado las superficies a la solicitud de modificación de centro de faenamiento.";
                    Content_msgGrilla.Visible = true;
                }
                else
                {
                    msgGrilla.Text = "No se han guardado las superficies a la solicitud de modificación de centro de faenamiento.";
                    Content_msgGrilla.Visible = true;
        ...
        protected void GridViewSolicitudesPendientes_PageIndexChanged(object sender, GridViewPageEventArgs e)
        {
            ...
            GridViewSolicitudesPendientes.DataSource = solicitudModificacionService.ListarRequerimientosPendModificacionCodCentro(solicitudModificacion.tramiteModConcesion.centro.id, solicitudModificacion.idSolConcesion, rbTipo.TIPO_TRAMITE_SOLICITUD_MOD_CENTRO_FAENAMIENTO, rbTipo.UNID_ESPACIAL_MOD_CENTRO_FAENAMIENTO);
            GridViewSolicitudesPendientes.DataBind();
