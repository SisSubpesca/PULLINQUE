...
namespace Validaciones.cl.subpesca.rb.solicitud
{
    ...
    /**
     * Clase que valida cada una de las secciones del formulario Antecedentes del Sector
     */
    public class AntecedDelSectorValidacion
    {
        ...
        public List<String> validaUbicacionGeografica(Datos.Entidades.SolicitudConcesion solicitudConcesion)
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                if (solicitudConcesion.cuerpoAgua != null && solicitudConcesion.cuerpoAgua.tipoCuerpoAgua != null && solicitudConcesion.cuerpoAgua.tipoCuerpoAgua.id > 0 && solicitudConcesion.cuerpoAgua.idCuerpoDeAgua < 1)
                {
                    listaErrroresUbicacionGeografica.Add("Si selecciona Tipo de Cuerpo de Agua, el Cuerpo de Agua es de ingreso obligatorio.");
                }
        ...
        public List<String> validaBarrio(Datos.Entidades.SolicitudConcesion solicitudConcesion)
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                /* Para guardar Barrio debe guardar primero la región */
                if (solicitudConcesion.region == null || solicitudConcesion.region.id <= 0)
                {
                    listaErrroresBarrio.Add("Debe guardar primero Ubicación Geográfica para guardar Agrupación de Concesiones.");
                }
        ...
        public List<string> validaVertice(Vertice vertice, List<Vertice> List_Vertices)
        {
            ...
            if (vertice != null)
            {
               /* No se pueden ingresar vértices con las mismas letras */
                ...
                if (vertice.latitudHora < 0 || vertice.latitudHora > 90) {
                    listaErrroresVertice.Add("Latitud Grado debe ser entero y estar entre el rango 0 a 90.");
                }

                if (vertice.latitudMinuto < 0 || vertice.latitudMinuto > 59)
                {
                    listaErrroresVertice.Add("Latitud Minuto debe ser entero y estar entre el rango 0 a 59.");
                }


                if (vertice.longitudHora < 0 || vertice.longitudHora > 360)
                {
                    listaErrroresVertice.Add("Longitud Grado debe ser entero y estar entre el rango 0 a 360.");
                }

                if (vertice.longitudMinuto < 0 || vertice.longitudMinuto > 59)
                {
                    listaErrroresVertice.Add("Longitud Minuto debe ser entero y estar entre el rango 0 a 59.");
                }
        ...
        public List<string> validaReferenciasGeograficas(SolicitudConcesion solicitudConcesion)
        {
            ...
            if (solicitudConcesion != null)
            {
                ...
                if (solicitudConcesion.coordenadaGeografica != null && solicitudConcesion.coordenadaGeografica[0].idCoordenadaGeo > 0)
                {
                    ...
                    /* La Carta a almacenar debe estar en el sistema */
                    if (solicitudConcesion.coordenadaGeografica[0].tipoCoordgeografica != null && (solicitudConcesion.coordenadaGeografica[0].tipoCoordgeografica.id == rbTipo.REGULARIZACION ||
                        solicitudConcesion.coordenadaGeografica[0].tipoCoordgeografica.id == rbTipo.ANTECEDENTES_ESPACIALES))
                    {
                        if (solicitudConcesion.coordenadaGeografica[0].carta != null && solicitudConcesion.coordenadaGeografica[0].carta.idCarta <= 0)
                        {
                            listaErrroresReferenciasGeograficas.Add("Debe ingresar Carta válida para la Coordenada Geográfica.");
                        }
                    }
